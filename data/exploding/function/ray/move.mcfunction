...
execute as @s[tag=!hit_block] unless block ^0.7 ^ ^ #exploding:ray_permeable run scoreboard players add @s RayHitL 1
execute as @s[tag=!hit_block] unless block ^-0.7 ^ ^ #exploding:ray_permeable run scoreboard players add @s RayHitR 1
execute as @s[tag=!hit_block] unless block ^ ^0.7 ^ #exploding:ray_permeable run scoreboard players add @s RayHitU 1
execute as @s[tag=!hit_block] unless block ^ ^-0.7 ^ #exploding:ray_permeable run scoreboard players add @s RayHitD 1
...
# If block was hit
execute if entity @s[tag=hit_block,tag=!hit_block_check] run function exploding:explode

# Recurse until we hit a block or run out of steps
execute as @s[tag=!hit_block,tag=!hit_block_check,scores={RaycastSteps=1..}] positioned ^ ^ ^0.25 run function exploding:ray/move
